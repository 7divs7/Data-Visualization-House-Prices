...
const margin = {top: 10, right: 70, bottom: 30, left: 80},
...
d3.csv("../datasets/combined-dataset-main.csv").then( function(data) {
  ...
  const y2 = d3.scaleLinear()
    .domain([0, d3.max(data, d => +d.Income)]) // Update with appropriate data property
    .range([height, 0]);
  svg.append("g")
    .attr("class", "axis")
...
svg.append("g")
  ...
  .attr("cy", "310")
  .attr("r", 7)  // Set radius
  .style("fill", "#f7bf50");  // Set fill color
svg.append("text")
  .attr("transform", "rotate(-90)")
  .attr("y", 830 - margin.right)
  .attr("x", 0 - (height / 2))
  .attr("dy", "1em")
  .style("text-anchor", "middle")
  .text("Average Household Income (in £) "); 
   ...
   .attr("d", d3.line()
      .x(function(d) { return x(d.Year); })
      .y(function(d) { return y2(+d.Income); })
   )
   .attr("stroke", "#f7bf50")
   .style("stroke-width", 4)
   .style("fill", "none");
   ...
   svg.selectAll('.circle2')
     ...
     .attr('class', 'circle2')
     .attr('cx', d => x(d.Year)) // Use the x scale to position circles based on Year
     .attr('cy', d => y2(d.Income))
     .attr('r', 5)
     .attr('fill', "#f7bf50") // Add fill color for circles
     .on('mouseover', function(event, d) {
       const [x, y] = d3.pointer(event, this);
       const x1 = x + 10; // Adjust x position of tooltip
       const y1 = y + 50; // Adjust y position of tooltip
       const text = `<b>${d.Year}</b><br>Average Household Income (in £): ${d.Income}`;
       tooltip.transition().duration(200).style("opacity", 0.9);
       tooltip.html(text)
           .style("left", `${x1}px`)
           .style("top", `${y1}px`)
       tooltip.style("background-color", "#f7bf50")
       tooltip.style("display", "block");
   })
   .on('mouseout', function() {
       tooltip.transition().duration(200).style("opacity", 0);
